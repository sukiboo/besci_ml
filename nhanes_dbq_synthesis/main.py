...
if __name__ == '__main__':
    ...
    train_data, test_data = split_data(data, val_split=0.1, seed=seed)

    # train ml model
    model, loss = get_model(train_data, test_data, seed=seed)
    print(f'model is trained: loss = {loss[0][-1]:.2e} / {loss[1][-1]:.2e}')
    ##plot_loss(loss)

    # extrapolate data
    dems = synthesize_demographics(num=10000, seed=seed)
    pref = synthesize_preferences(dems, seed=seed)
